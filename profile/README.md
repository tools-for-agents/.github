...
| [**prism**](https://github.com/tools-for-agents/prism) | 🔻 The agent's data reader — any JSON, JSONL, CSV or TSV blob becomes its *shape* and the *slice* you asked for, never the whole thing — shape, read, find or **diff**. Byte-, depth-, token- and node-bounded, because it reads untrusted data. CLI + web + MCP. Zero dependencies. |
